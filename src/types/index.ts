...
export interface TutorialStageConfig {
    ...
    description: string;
    hint: string;
    allowedActions: readonly string[]; // e.g., ['piece_select', 'card_select', 'move']
    requiredAction?: {
        type: string; // 'piece_select', 'card_select', 'move'
        validation: (data: any) => boolean;
    };
    boardSetup?: {
        pieces?: Array<{ player: Player; type: PieceType; row: number; col: number }>;
        playerCards?: {
            red: readonly [MovementCard, MovementCard];
            blue: readonly [MovementCard, MovementCard];
        };
...
export type TutorialGameState = {
    currentStage: TutorialStageName;
    stageIndex: number;
    game: GameState;
    ui: UIState;
